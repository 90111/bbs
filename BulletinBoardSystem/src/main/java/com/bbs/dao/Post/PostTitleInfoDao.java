...
public interface PostTitleInfoDao {
    ...
    @Select("SELECT id, title, view_num, like_num, image FROM PostTitleInfo WHERE owner=#{id} ORDER BY `post_time` DESC limit 2")
    List<PostTitleInfo> getUserRecentPostTitleByUserId(int id) throws Exception;

    @Select("SELECT id, title, view_num, like_num, image FROM PostTitleInfo WHERE owner=#{id} ORDER BY `post_time` DESC")
